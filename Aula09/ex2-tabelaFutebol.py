...
def main():
    ...
    print()
    print("Ordenada pela diferença entre Golos Marcados e Sofridos")
    printTabela(tab)
    ...
    tab = sorted(tabela , key= lambda x : (pontos(x) , x[GM] - x[GS]) , reverse=True) #key = (first , second(if equal))
    print()
    print("Ordenada por pontos, e se iguais, pela diferença entre Golos Marcados e Sofridos")
    printTabela(tab)
