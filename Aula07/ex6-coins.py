...
def value(bag):
    ...
    amount = 0
    for coinType in bag:
        coinValue = bag[coinType]
        amount += coinType*coinValue
    return amount
...
def transfer(bag1, amount, bag2):
    ...
    if value(bag1) < amount:
        ...
        while amount > 0:
            for coin in list(bag1.keys())[::-1]:
                while coin <= amount and bag1[coin] > 0:
                    bag1[coin] -= 1
                    amount -= coin
                    if coin not in bag2:
                        bag2[coin] = 1
